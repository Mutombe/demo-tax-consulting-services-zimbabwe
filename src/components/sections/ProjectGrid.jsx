...
function MasonryProjects() {
  ...
  return (
    <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
      {featuredProjects.map((project, index) => (
        <SectionReveal key={project.title} delay={index * 0.15}>
          <Link to="/projects" className={`group relative block rounded-2xl overflow-hidden ${heights[index % heights.length]} break-inside-avoid`}>
            <img
              src={project.image}
              alt={`${project.title} by ${business.name}`}
...
function BentoProjects() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      {/* Large item */}
      <SectionReveal>
        <Link to="/projects" className="group relative block rounded-2xl overflow-hidden aspect-[4/5] md:row-span-2">
          <img
