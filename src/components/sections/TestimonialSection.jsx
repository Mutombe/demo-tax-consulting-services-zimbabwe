...
import { Star, Quotes, ArrowRight, ArrowLeft, CaretLeft, CaretRight } from '@phosphor-icons/react';
      ...
      <div className="max-w-7xl mx-auto">
        <SectionReveal>
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-navy-900 mb-6">What Our Clients Say</h2>
          ...
          {homeTestimonials.map((item, index) => (
            <SectionReveal key={item.name} delay={index * 0.15}>
              <div className="bg-earth-50/50 border border-earth-100 rounded-2xl p-8 h-full flex flex-col">
                <Quotes size={32} weight="fill" className="text-gold-500/30 mb-4" />
                <p className="text-steel-700 leading-relaxed flex-1 mb-6">"{item.text}"</p>
        ...
        <div className="relative">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="bg-earth-50 border border-earth-100 rounded-2xl p-10 text-center"
            >
              <Quotes size={48} weight="fill" className="text-gold-500/20 mx-auto mb-6" />
              <p className="text-xl text-steel-700 leading-relaxed mb-8 max-w-2xl mx-auto">"{item.text}"</p>
          ...
          <div className="flex justify-center gap-4 mt-8">
            <button onClick={goPrev} className="w-10 h-10 rounded-full border border-earth-200 flex items-center justify-center hover:bg-earth-50 transition-colors">
            ...
            <div className="flex items-center gap-2">
              {homeTestimonials.map((_, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all ${i === current ? 'w-6 bg-gold-500' : 'w-2 bg-earth-200'}`}
                />
              ))}
            </div>
            <button onClick={goNext} className="w-10 h-10 rounded-full border border-earth-200 flex items-center justify-center hover:bg-earth-50 transition-colors">
    ...
    <section className="section-padding bg-navy-900">
      <div className="max-w-5xl mx-auto text-center">
        <SectionReveal>
          ...
          <Quotes size={64} weight="fill" className="text-gold-500/20 mx-auto mb-8" />
          <blockquote className="text-2xl sm:text-3xl lg:text-4xl text-white font-light leading-relaxed mb-10 max-w-3xl mx-auto">
            "{featured.text}"
          </blockquote>
          <div className="flex items-center justify-center gap-1 mb-3">
            {[...Array(featured.rating)].map((_, i) => (
              <Star key={i} size={20} weight="fill" className="text-gold-400" />
            ))}
          </div>
          ...
          <div className="mt-16 grid sm:grid-cols-2 gap-6">
            {homeTestimonials.slice(1).map((item, index) => (
              <SectionReveal key={item.name} delay={index * 0.1}>
                <div className="text-left border border-white/10 rounded-xl p-6">
                  <p className="text-white/70 text-sm leading-relaxed mb-4">"{item.text}"</p>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-gold-500/20 rounded-full flex items-center justify-center text-gold-400 font-semibold text-xs">
                      {item.name.charAt(0)}
                    </div>
                    <div>
    ...
    <section className="section-padding bg-white">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-8">
          {homeTestimonials.map((item, index) => (
            <SectionReveal key={item.name} delay={index * 0.1}>
              <div className="border-l-3 border-gold-500 pl-8 py-2">
