          ...
          {stats.map((stat, index) => {
            ...
            return (
              <SectionReveal key={stat.label} delay={index * 0.1}>
                <div className={`text-center py-8 px-4 ${index < stats.length - 1 ? 'border-r border-gray-100' : ''}`}>
                  <IconComp size={28} className="text-gold-500 mx-auto mb-3" weight="fill" />
                  <div className="text-3xl sm:text-4xl font-bold text-navy-900 mb-1">{stat.number}</div>
          ...
          {stats.map((stat, index) => (
            <SectionReveal key={stat.label} delay={index * 0.1}>
              <div className="text-center">
                <div className="text-4xl sm:text-5xl font-bold text-navy-900 mb-2">{stat.number}</div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, index) => {
            ...
            return (
              <SectionReveal key={stat.label} delay={index * 0.1}>
                <div className="bg-earth-50 border border-earth-100 rounded-2xl p-6 text-center hover:shadow-lg transition-shadow">
    ...
    <section className="py-16 bg-navy-900" ref={ref}>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="space-y-8">
          {stats.map((stat, index) => {
            // Extract numeric part for bar width
              ...
              <SectionReveal key={stat.label} delay={index * 0.1}>
                <div>
                  <div className="flex justify-between items-baseline mb-2">
                    ...
                    <span className="text-gold-400 text-2xl font-bold">{stat.number}</span>
                  </div>
